...
import { parseRdfInput, serializeRdfStore, RDF_FORMATS } from './rdf-io.js';
...
/** @typedef {import('./rdf-io.js').ParsedOntology} ParsedOntology */
...
async function loadSelectedOntologyForNlpQa() {
  ...
  try {
    ...
    parsedOntology = await parseRdfInput(text, file.name);
...
async function downloadCurrentNlpQaOntologyAsTurtle() {
  ...
  try {
    const text = await serializeRdfStore(parsedOntology.store, RDF_FORMATS.TURTLE, {
    ...
    downloadTextFile(text, `${stem}_nlp-qa_${getTimestampForFileName()}.ttl`, 'text/turtle;charset=utf-8');
